...
PROGNAME="librespeedtest"
BINARY=${PROGNAME}-$(go env GOOS)-$(go env GOARCH)
BUILD_DATE=$(date -u "+%Y-%m-%d %H:%M:%S %Z")
LDFLAGS="-w -s -X \"librespeedtest/defs.ProgName=${PROGNAME}\" -X \"librespeedtest/defs.ProgVersion=${PROGVER}\" -X \"librespeedtest/defs.BuildDate=${BUILD_DATE}\""
