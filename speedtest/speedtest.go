...
func SendTelemetry(
	...
	if fIspInfo, err := wr.CreateFormField("ispinfo"); err != nil {
		logrus.Debugf("Error creating form field: %s", err)
		return "", err
	} else if _, err = fIspInfo.Write(b); err != nil {
		logrus.Debugf("Error writing form field: %s", err)
		return "", err
	}
	...
	if err := wr.Close(); err != nil {
		logrus.Debugf("Error flushing form field writer: %s", err)
		return "", err
	}
	...
	if err != nil {
		logrus.Debugf("Error when creating HTTP request: %s", err)
		return "", err
	}
	...
	if err != nil {
		logrus.Debugf("Error when making HTTP request: %s", err)
		return "", err
	}
	fmt.Println(resp)
	...
	if err != nil {
		logrus.Errorf("Error when reading HTTP request: %s", err)
		return "", err
	}
